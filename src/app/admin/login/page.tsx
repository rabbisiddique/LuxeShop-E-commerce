...
export default function AdminLoginPage() {
  ...
  const [showError, setShowError] = useState(true); // Static design shows error state by default for demonstration
      ...
      <div className="relative z-10 flex flex-col items-center">
        ...
        <div className="bg-[#1F1F1F] border border-[#2A2A2A] rounded-[20px] p-10 w-[420px] shadow-[0_24px_64px_rgba(0,0,0,0.4)]">
          ...
          <header className="flex flex-col items-center text-center mb-8">
            ...
            <p className="text-[13px] text-[#9A9A9A] mt-1.5 font-medium">LuxeShop Management System</p>
          </header>
          ...
          <div className="flex justify-center mb-6">
            <div className="inline-flex items-center gap-2 bg-[#2A2A2A] border border-[#3A3A3A] rounded-full px-4 py-2">
              ...
              <span className="text-[12px] text-[#9A9A9A] font-medium tracking-wide">Authorized personnel only</span>
            </div>
          </div>
          ...
          <form className="space-y-4" onSubmit={(e) => e.preventDefault()}>
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5 ml-1">
                ...
                <button type="button" className="text-[12px] text-[#FF6B35] font-semibold hover:underline transition-all">
                  Forgot password?
                </button>
              </div>
              <div className="relative group">
                <Lock className="absolute left-4 top-1/2 -translate-y-1/2 text-[#9A9A9A] group-focus-within:text-[#FF6B35] transition-colors" size={16} />
                <input 
            ...
            {/* Error State */}
            {showError && (
              <div className="bg-[#2A0000] border border-[#DC2626]/50 rounded-lg p-3.5 flex items-start gap-2.5 mt-4">
                <AlertCircle className="text-[#DC2626] shrink-0 mt-0.5" size={14} />
                <p className="text-[13px] text-[#DC2626] font-medium leading-tight">Invalid credentials. Try again.</p>
              </div>
            )}
        ...
        <div className="mt-6 flex items-center justify-center gap-2 text-[#4B4B4B]">
          <Lock size={12} />
          <span className="text-[12px] font-medium tracking-tight">Secured by 256-bit SSL encryption</span>
        </div>
      </div>
    </main>
  );
