...
import { Mail, Lock, Eye, EyeOff, User, ShoppingBag, CheckCircle, AlertCircle, Loader2, ArrowLeft } from 'lucide-react';
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#FAFAFA] p-6">
        ...
        >
          ...
          <h1 className="font-['Playfair_Display'] text-[28px] font-bold text-[#0D0D0D] mb-3">Account Created!</h1>
          ...
          <div className="space-y-4">
            <p className="text-[12px] text-[#9A9A9A]">Redirecting to login page...</p>
            <Link 
              href="/login" 
    ...
    <div className="min-h-screen grid grid-cols-1 lg:grid-cols-[40%_60%] bg-[#FAFAFA]">
      ...
      >
        ...
        <div className="relative z-10 flex flex-col items-center text-center max-w-md mx-auto">
          ...
          >
            Create your account and unlock exclusive deals, fast checkout, and order tracking.
          </motion.p>
      ...
      <div className="flex flex-col justify-center px-6 py-8 sm:px-12 lg:px-24 xl:px-32 overflow-y-auto relative">
        ...
        >
          <header className="mb-7">
            <h1 className="font-['Playfair_Display'] text-[30px] font-bold text-[#0D0D0D]">Create your account</h1>
            <p className="text-[14px] text-[#9A9A9A] mt-2">Join LuxeShop today &mdash; it&apos;s free.</p>
          </header>
          ...
          <form onSubmit={handleSubmit(handleRegister)} className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-[13px] font-semibold text-[#4B4B4B] mb-1.5">First Name</label>
                <div className="relative group">
                  <User className={`absolute left-4 top-1/2 -translate-y-1/2 transition-colors ${errors.firstName ? 'text-[#DC2626]' : 'text-[#9A9A9A] group-focus-within:text-[#FF6B35]'}`} size={16} />
                  <input 
                    type="text" 
                    {...register('firstName')}
                    placeholder="Sarah"
                    className={`w-full h-[44px] bg-white border-[1.5px] rounded-lg pl-[42px] pr-4 text-[14px] outline-none transition-all ${
                      errors.firstName ? 'border-[#DC2626] focus:ring-4 focus:ring-[#DC2626]/10' : 'border-[#E8E8E8] focus:border-[#FF6B35]'
                    }`}
                  />
                </div>
                {errors.firstName && <p className="text-[11px] text-[#DC2626] mt-1 font-medium">{errors.firstName.message}</p>}
              </div>
              <div>
                <label className="block text-[13px] font-semibold text-[#4B4B4B] mb-1.5">Last Name</label>
                <div className="relative group">
                  <User className={`absolute left-4 top-1/2 -translate-y-1/2 transition-colors ${errors.lastName ? 'text-[#DC2626]' : 'text-[#9A9A9A] group-focus-within:text-[#FF6B35]'}`} size={16} />
                  <input 
                    type="text" 
                    {...register('lastName')}
                    placeholder="Johnson"
                    className={`w-full h-[44px] bg-white border-[1.5px] rounded-lg pl-[42px] pr-4 text-[14px] outline-none transition-all ${
                      errors.lastName ? 'border-[#DC2626] focus:ring-4 focus:ring-[#DC2626]/10' : 'border-[#E8E8E8] focus:border-[#FF6B35]'
                    }`}
                  />
                </div>
                {errors.lastName && <p className="text-[11px] text-[#DC2626] mt-1 font-medium">{errors.lastName.message}</p>}
              </div>
            </div>

            <div>
              <label className="block text-[13px] font-semibold text-[#4B4B4B] mb-1.5">Email Address</label>
              <div className="relative group">
                ...
                <input 
                  ...
                  className={`w-full h-[44px] bg-white border-[1.5px] rounded-lg pl-[42px] pr-10 text-[14px] outline-none transition-all ${
                    errors.email ? 'border-[#DC2626] focus:ring-4 focus:ring-[#DC2626]/10' : 'border-[#E8E8E8] focus:border-[#FF6B35]'
                  }`}
                />
              </div>
              {errors.email && <p className="text-[11px] text-[#DC2626] mt-1 font-medium">{errors.email.message}</p>}
            </div>

            <div>
              <label className="block text-[13px] font-semibold text-[#4B4B4B] mb-1.5">Password</label>
              <div className="relative group">
                ...
                <input 
                  ...
                  className={`w-full h-[44px] bg-white border-[1.5px] rounded-lg pl-[42px] pr-12 text-[14px] outline-none transition-all ${
                    errors.password ? 'border-[#DC2626] focus:ring-4 focus:ring-[#DC2626]/10' : 'border-[#E8E8E8] focus:border-[#FF6B35]'
                  }`}
                />
                <button 
                  type="button"
              ...
              <div className="mt-2.5">
                <div className="flex items-center justify-between mb-1.5">
                  <div className="flex gap-1 flex-1">
                    <div className={`h-1 rounded-full flex-1 transition-colors ${strength >= 1 ? 'bg-[#FF6B35]' : 'bg-[#E8E8E8]'}`} />
                    <div className={`h-1 rounded-full flex-1 transition-colors ${strength >= 2 ? 'bg-[#FF6B35]' : 'bg-[#E8E8E8]'}`} />
                    <div className={`h-1 rounded-full flex-1 transition-colors ${strength >= 3 ? 'bg-[#16A34A]' : 'bg-[#E8E8E8]'}`} />
                    <div className={`h-1 rounded-full flex-1 transition-colors ${strength >= 4 ? 'bg-[#16A34A]' : 'bg-[#E8E8E8]'}`} />
                  </div>
                  <span className={`text-[11px] font-semibold ml-3 transition-colors ${strength >= 3 ? 'text-[#16A34A]' : 'text-[#9A9A9A]'}`}>
                    {strength === 0 ? 'Weak' : strength === 1 ? 'Fair' : strength === 2 ? 'Good' : 'Strong'}
                  </span>
                </div>
                {errors.password && <p className="text-[11px] text-[#DC2626] mt-1 font-medium">{errors.password.message}</p>}
              </div>
            </div>

            <div>
              <label className="block text-[13px] font-semibold text-[#4B4B4B] mb-1.5">Confirm Password</label>
              <div className="relative group">
                <Lock className={`absolute left-4 top-1/2 -translate-y-1/2 transition-colors ${errors.confirmPassword ? 'text-[#DC2626]' : 'text-[#9A9A9A] group-focus-within:text-[#FF6B35]'}`} size={16} />
                <input 
                  type="password"
                  {...register('confirmPassword')}
                  placeholder="••••••••"
                  className={`w-full h-[44px] bg-white border-[1.5px] rounded-lg pl-[42px] pr-4 text-[14px] outline-none transition-all ${
                    errors.confirmPassword ? 'border-[#DC2626] focus:ring-4 focus:ring-[#DC2626]/10' : 'border-[#E8E8E8] focus:border-[#FF6B35]'
                  }`}
                />
              </div>
              {errors.confirmPassword && <p className="text-[11px] text-[#DC2626] mt-1 font-medium">{errors.confirmPassword.message}</p>}
            </div>
            ...
            <div className="flex items-start gap-2 mt-4 mb-6">
              <label className="flex items-start gap-2 cursor-pointer group pt-0.5">
                <div className="relative flex items-center justify-center shrink-0">
                  ...
                  <div className={`w-4 h-4 border-[1.5px] rounded transition-all ${errors.terms ? 'border-[#DC2626]' : 'border-[#E8E8E8] peer-checked:bg-[#FF6B35] peer-checked:border-[#FF6B35]'}`} />
                ...
                <span className="text-[13px] text-[#4B4B4B] leading-tight">
                  I agree to the{' '}
                  <Link href="#" className="text-[#FF6B35] font-medium hover:underline">Terms of Service</Link>{' '}
                  and{' '}
                  <Link href="#" className="text-[#FF6B35] font-medium hover:underline">Privacy Policy</Link>
                </span>
              </label>
            </div>
          ...
          <div className="text-center mt-6">
            <p className="text-[13px] text-[#9A9A9A]">
              ...
              <Link href="/login" className="font-semibold text-[#FF6B35] hover:underline">Sign in →</Link>
            </p>
          </div>
        </motion.div>
      </div>
